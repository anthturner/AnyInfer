...
| D34 | Provider-declared token calibration (2026-08-08, amends D25) | `ProviderDescriptor.token_calibration: TokenCalibration(multiplier, overhead_tokens)` corrects for a provider that bills for an envelope the caller never serialized — a session harness, an agent preamble, service-side tool declarations. D25's calibration knob lived on the *estimator*, which is chosen once per client, so it could not express a per-provider fact; a Copilot route was budgeted with the same bytes/3 rule as raw HTTP and under-counted every request systematically. `RequestEstimate` gains an `envelope` component so the correction is visible rather than smeared across the others, and the correction moves the **planning figure only**: the floor is what the L6 gate refuses on, and a lower bound may only claim tokens the provider certainly charges. Shipped non-identity for `copilot` alone (2.4× + 1,200 tokens, the figures Frisket arrived at against the same SDK); every other descriptor is the identity, which is also the default, so a provider that counts what it was sent says nothing. |

| D35 | Provider-declared repair ceiling (2026-08-08, amends D6) | `ProviderDescriptor.max_repair_attempts: int | None` caps how many schema-repair round trips a provider may be asked for; the core clamps `Repair.max_attempts` to it. The repair budget stays the caller's decision everywhere it can be honored — the ceiling exists for providers where a re-ask costs far more than the malformed answer is worth, and shipped non-null only for `m365-copilot` (1), whose every request is a Graph call against a service-held conversation behind interactive auth. The clamp is reported as `ParameterDropped(parameter="repair.max_attempts")` rather than performed silently: a budget quietly reduced from three to one is no more discoverable than a parameter ignored outright (L5). `ParameterDropped`'s contract widens from "was not sent" to "was not honored as asked" to cover partial honoring; no new event type, so the ADR-006 union and its OTel mapping are unchanged. |

| D36 | Provider runtime diagnostics (2026-08-08, resolves open question 6; implements the DESIGN.md §8 "declared, not duck-typed" rule) | A third channel beside errors and capabilities: an error stops a request and a capability describes a model, but neither covers *the request succeeded and something about how it ran is worth saying*. `Diagnostic(code, severity, message)` is content-free and advisory; adapters implement `SupportsDiagnostics.diagnostics()` and **declare** it via `ProviderDescriptor.reports_diagnostics` — the flag is the switch, not the presence of the method, so which providers can answer is readable from the registry rather than from `hasattr` (improving on Frisket's `getattr`-probed `provider_runtime_diagnostics`). Collected after a successful attempt onto `Generation.warnings` *and* emitted as `ProviderDiagnostic` telemetry (correlated when collected in a request, standalone otherwise, per the ADR-006 no-`request_id` rule); also callable directly as `client.diagnostics(provider_id)`. Every failure mode — declared-but-unimplemented, raising, wrong return type — resolves to "nothing to report": advisory data must never turn a successful generation into a failed one. **Resolves open question 6** in favor of adapter-reported diagnostics rather than the capability layer: VRAM residency is a fact about a running process at a moment in time, not about a model, and `Sourced[T]` has nowhere honest to put it. Shipped: `ollama` (`ollama.gpu-spill` from `/api/ps`, 95% residency threshold) and `llama-cpp` (`llama-cpp.cpu-only` when a plan offloads no layers on a machine with an accelerator, read from supervisor state so it never triggers hardware detection). |

| D37 | End-to-end target verification (2026-08-08) | `client.verify(target)` → `Verification`, plus `anyinfer verify`. A health probe answers "can I reach this endpoint", which is not the question behind a *Test connection* button: a credential can be valid for a model listing and not for inference, a model id can be a typo, a deployment can exist with no capacity, and a provider can answer fluently while never holding a schema. The probe spends one bounded request (64 output tokens, trivial schema, temperature 0) and **reports rather than raises** — "this target is broken" is the answer, not a failure to answer; only an unresolvable *target* still raises `ConfigError`. Two booleans, not one: `reached` (it answered) and `ok` (it answered correctly), because unreachable and reachable-but-cannot-hold-a-shape need different fixes. No retry and no fallback — a chain that quietly answered from elsewhere would report a connection the operator does not have. Grading is lenient about packaging and strict about content ("Ok!" passes, "sure thing" does not). Composes D36: the report carries the target's runtime diagnostics, so *works slowly* is distinguishable from *works*. The probe prompt is mechanical scaffolding for a library-owned operation, the same category as the schema-repair re-prompt, and does not reopen the §2 "no prompt templating" non-goal. Ported in spirit from Frisket `providers/health.py`. |

| D38 | Active capability probes (2026-08-08, implements the DESIGN.md §7 third assembly layer; ADR-005's `probed` tier finally has a producer) | `capabilities/probes.py` + `client.probe(target)` → `ProbeReport`. `CapabilityStore.record_probe` had existed since M0 with no caller: the layer was designed, typed, and never fed. The gap it closes is the compatibility surface — eighty-six presets and every self-hosted OpenAI-compatible server inherit a descriptor's *default*-provenance feature guess, and a server that accepts `response_format` while ignoring it is indistinguishable from one that honors it until a schema silently stops being enforced. Each probe sends one trivial request with exactly one mechanism forced (via synthetic capabilities handed to the wire builder — the point is to force what the ladder would not choose) straight at the adapter, unrouted: a retry or fallback answering from elsewhere would record a measurement of the wrong model. **Three outcomes, not two:** `supported`, `unsupported`, and `inconclusive` for a provider that accepted the request and answered something else — one reply cannot separate a weak model from an ignored parameter, and only conclusive probes record. `probed_features()` folds results into the *complete* flag because `features` overlays wholesale; recording a bare `Feature.TOOLS` would erase every other bit. Probeable: JSON_SCHEMA, JSON_MODE, GRAMMAR, TOOLS, STREAMING; default set is four requests. Excluded by design: REASONING (accepted-and-ignored nearly always ⇒ perpetually inconclusive), SYSTEM_PROMPT/CACHE_USAGE (unreadable from one reply), and numeric bounds (bisecting a context window costs dozens of requests to learn what a catalog row already says). |

| D39 | Throughput measurement and caller-owned measurement store (2026-08-08) | `client.benchmark(target)` → `Measurement`, plus `anyinfer benchmark`. Capabilities describe a model; none of them says how fast it is *here*, and for local inference that is the number that decides everything — the same weights on the same GPU differ by an order of magnitude with what else is resident and how many layers were offloaded. **Prefill and decode are reported separately** (compute-bound vs bandwidth-bound; one figure would hide the distinction that matters), and `prefill_tokens_per_s` is `None` unless the provider timed its own prefill phase — deriving it from TTFT would fold queueing and network latency into a figure labelled compute, which is the tri-state discipline applied to time instead of money. Deterministic prompt so two runs differ because the machine changed, not the prompt. Unrouted, like D37/D38. **Persistence is opt-in and caller-pointed**: `MeasurementStore(path)` writes only where asked, keeping the §14 "nothing is written by default" rule intact — the library never picks a location. `MeasurementIdentity` (provider, model, normalized endpoint, machine signature, runtime) has a sha256 `fingerprint` that keys the store, so a measurement from another endpoint or machine is a *different* measurement rather than a fresher one; endpoint normalization strips userinfo/query/fragment, so nothing credential-shaped reaches disk. A hosted target records no machine signature — this computer's specs say nothing about someone else's throughput. Reads are total: a corrupt, foreign, or future-version store reads as empty, because a cache that can break a program is worse than no cache. Generalizes Frisket `providers/benchmark.py`; the identity/fingerprint idea is ModelFit `identity.py`. |

| D40 | Session reuse API (2026-08-08, resolves open question 2 / DESIGN.md §20 #2) | `client.session(target)` → `Session`, passed to `generate()`/`stream()`. The three providers that can carry state save *different* things — Copilot keeps the conversation server-side, llama.cpp keeps a pinned server and its KV cache, Ollama keeps the model resident — so the shared abstraction is a **shape** ("these requests belong together"), not a protocol: `Session` is an opaque handle and the core never interprets what a provider stores in it. Three invariants: **a session never changes an answer** (it is a performance/cost optimization, so passing one to a stateless provider is legal and inert, reported as `supported=False`), **state is bound to one target** (after a fallback the handle means nothing there, so that turn runs stateless and reports `unsupported` rather than shipping one provider's token to another), and **the provider decides** (`reuse` reports what actually happened — a resumed session the provider had expired reports `fresh`). Wire plumbing is tri-state: `WireRequest.session_state` is `None` for no session and `{}` for a session's *first* turn, a distinction an empty mapping alone cannot make. `AdapterFinal.session_state` carries what to remember. `Session` is a mutable handle rather than a frozen value — it is a stream-like object, not a domain type — and a session with no explicit target stands in as the route. Closing is deliberately **local**: server-side state expires on the provider's own schedule, and reaching out to "end" it would promise something no provider guarantees. Implemented for `copilot` (SDK session held across turns; a resumed turn sends only the newest user message instead of re-folding the history, which is the actual saving), `llama-cpp` (supervisor `persist=True` so idle collection cannot discard the KV cache mid-conversation), and `ollama` (`keep_alive`, with an explicitly-configured value always winning — it is the caller's memory). |

| D41 | Engine-managed model acquisition — **partial reversal of D31** (2026-08-08) | D31 put `ollama pull` deliberately out of scope. That was right about the *architecture* and wrong about the *need*: mote-cli's `Engine.warmup()` pulls, Frisket's tier provisioning assumes a pulled model, and an application configured against Ollama otherwise has to shell out to a binary AnyInfer already talks to over HTTP — a gap that made the sibling migration land on "use the library, except for this one thing". Recorded as a reversal rather than an extension because D31 named it explicitly. **The architectural reason for the exclusion is preserved, not overridden.** Acquisition still never lives in an adapter: `local/services.py` speaks `POST /api/pull` and the Ollama descriptor merely *points* at it through a new declarative `ProviderDescriptor.model_puller` hook, so which providers can do this is answerable from the registry rather than from engine checks in the core. It is also kept honest about what it is not: bytes land in the *engine's* store under the engine's own name, nothing is written to AnyInfer's model store, nothing is indexed, and `locate_model()` will not find them — which is why the surface is `pull_model()` ("make the engine ready") rather than a second spelling of `acquire_model()`. Per-layer counts are accumulated into the aggregate `DownloadProgress` figures the D31 amendment defined, since a per-layer counter restarting at zero is the exact defect that contract was rewritten to prevent. Mid-stream failures arrive as an `error` field on a 200 response, so both that and an HTTP status map onto the same typed errors; a missing model reads `pull model manifest: file does not exist`, which is a typo in a name rather than the disk error it looks like. Still out of scope from D31: supervising vLLM, local quantization, LoRA acquisition. LM Studio needs nothing here — it loads on first request. |

| D42 | Capability-gated parameters and a preflight dry run (2026-08-08, extends L5/D25) | Two small closures of the same principle. **(a) Reasoning is withheld from a model known not to have it.** A descriptor knows how a provider *spells* reasoning effort; only assembled capabilities know which of its models have one, so `build_wire_request` now consults `Feature.REASONING` and `dropped_parameters` reports the withholding as `ParameterDropped`. Gated on *known* absence only — a `default`-provenance feature set is a descriptor-level guess, and dropping a caller's parameter on a guess is worse than sending one the model ignores (the pre-dispatch gate's rule, applied to parameters). **(b) `anyinfer run --dry-run`** reports the estimate breakdown, window, allowance, remaining, fit, and cost range without sending, using `client.budget()` — the same calculator the real request is held to, so it is not a second estimate of the same thing. Unknown figures print `unknown` rather than a plausible default. Generalizes mote-cli `runtime/preflight.py`, minus its ETA (which needs measured throughput; `client.benchmark()` is where that data now comes from). |
...
2. ~~Session/conversation reuse API (mote token-cache: Copilot resume, Ollama keep_alive).~~ **Resolved as D40** (2026-08-08): an opaque, target-bound handle that never changes an answer; the three capable providers each exploit it differently behind one shape.
...
6. ~~Where Ollama GPU-spill/observed-VRAM checks live (adapter warnings vs capability layer).~~ **Resolved as D36** (2026-08-08): adapter-reported runtime diagnostics, declared on the descriptor. Residency is a fact about a running process at a moment in time, not about a model, so the capability layer had nowhere honest to put it.
...
- Open question 9 (code signing) remains open. Question 1 (token estimation) was
  resolved as D25 on 2026-08-07; question 2 (session reuse) as D40 and question 6
  (GPU-spill checks) as D36, both on 2026-08-08.
