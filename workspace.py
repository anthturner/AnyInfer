...
def render_service_install_text(executable: Path) -> str:
    """The ``INSTALL.txt`` shipped in the standalone sidecar archive.
    ...
    commands = "\n".join(f"  {' '.join(command)}" for command in definition.install_commands)
    notes = "\n".join(f"  - {note}" for note in definition.notes)
    return (
        "Running the AnyInfer sidecar as a managed service\n"
        "=================================================\n"
        "\n"
        f"This archive was built for {_bundle_platform_tag()}. The definition below "
        f"assumes\nthe executable lives at:\n\n  {executable}\n\n"
        "If you unpacked it somewhere else, do not edit this file — run\n\n"
        "  anyinfer-serve install --print\n\n"
        "which regenerates the same definition with the real path, and then\n\n"
        "  anyinfer-serve install\n\n"
        "to write and register it. Both bind 127.0.0.1 unless you pass\n"
        "--allow-remote-exposure together with a bearer token.\n"
        "\n"
        f"Definition ({definition.path}):\n"
        "\n"
...
def _write_service_install_text(app_dir: Path) -> None:
    """Drop the service-definition instructions beside the standalone sidecar."""
    executable = app_dir / ("anyinfer-serve.exe" if os.name == "nt" else "anyinfer-serve")
    (app_dir / "INSTALL.txt").write_text(
        render_service_install_text(executable.resolve()), encoding="utf-8"
    )
...
def _build_serve_bundle() -> int:
    ...
    _write_bundle_info(
        ...
        instructions=(
            ...
            "INSTALL.txt has the service definition that keeps it running across reboots."
        ),
    )
    _write_service_install_text(app_dir)
    executable = app_dir / ("anyinfer-serve.exe" if os.name == "nt" else "anyinfer-serve")
