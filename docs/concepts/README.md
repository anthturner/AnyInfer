...
| Page | The idea in one line |
|---|---|
| [Capabilities and provenance](capabilities.md) | Every capability value records where it came from, so a developer knows how much to trust it. |
...
| [The model catalog](catalog.md) | What a machine could run locally, whether this one can run it, and verified acquisition. |
...
When a developer changes `target=` from a hosted model to a local one, the behavior the
application depends on does not change with it.
