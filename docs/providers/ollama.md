...
Uses Ollama's native `/api/chat` API, not its `/v1` OpenAI-compatibility layer. The
native API carries grammar-enforced structured output, per-phase nanosecond timings,
`keep_alive` [session retention](../concepts/sessions.md), and reasoning via `think`;
the `/v1` layer does not implement all of these and drops the parameters it lacks
...
Model names may contain colons: `"ollama:qwen3:8b"` is the provider `ollama` and the
model `qwen3:8b`, because [targets](../concepts/targets.md) split on the first colon only.
...
Ollama's native `POST /api/embed` is a batch-capable embedding endpoint. (The older
`POST /api/embeddings` route is deprecated and singular-input; this adapter does not
speak it.)
