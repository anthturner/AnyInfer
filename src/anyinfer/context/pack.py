...
from .rank import build_rank_cache, score_document, tokenize
...
def reduce_packed(
    *,
    ...
    Args:
        ...
        ordered: The same documents in rank order (pinned first).
    ...
    for document in unpinned:
        chunks.extend(split_document(document, chunk_tokens=chunk_tokens))

    ranked_chunks = _rank_chunks(chunks, query, build_rank_cache(candidates))
    ...
    text = _render(whole, selected, render_order)
...
def _rank_chunks(chunks: list[Chunk], query: str, corpus: RankCache) -> list[Chunk]:
    ...
    ``credentials.py`` still benefits from its filename.
    """
    ...
    for chunk in chunks:
        ...
        scored.append(
            (chunk, score_document(surrogate, query_terms, _chunk_cache(surrogate, corpus)))
        )
...
def _chunk_cache(surrogate: ContextDocument, corpus: RankCache) -> RankCache:
    ...
    """
    tokens = tokenize(surrogate.content)
    return RankCache(
...
def _render(whole: list[ContextDocument], chunks: list[Chunk], render_order: str) -> str:
    """Render whole documents and coalesced chunk spans into one envelope."""
    blocks: list[tuple[str, str]] = [
        (document.path, render_file_block(document)) for document in whole
    ]
